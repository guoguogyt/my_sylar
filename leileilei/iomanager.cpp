...
IOManager::FdContext::EventContext& IOManager::FdContext::getContext(IOManager::Event event) {
    switch(event) {
        ...
        case IOManager::WRITE:
            return write;
        default:
            LEILEILEI_ASSERT2(false, "getContext");
    }
...
void IOManager::FdContext::triggerEvent(IOManager::Event event) {
    ...
    //if(LEILEILEI_UNLIKELY(!(event & event))) {
    //    return;
    //}
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if(LEILEILEI_UNLIKELY(fd_ctx->events & event)) {
        LEI_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd
                    << " event=" << (EPOLL_EVENTS)event
                    << " fd_ctx.event=" << (EPOLL_EVENTS)fd_ctx->events;
        LEILEILEI_ASSERT(!(fd_ctx->events & event));
    }
    ...
    if(rt) {
        LEI_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
    ...
    LEILEILEI_ASSERT(!event_ctx.scheduler
                && !event_ctx.fiber
                && !event_ctx.cb);
    ...
    if(cb) {
        ...
        LEILEILEI_ASSERT2(event_ctx.fiber->getState() == Fiber::EXEC
...
bool IOManager::cancelAll(int fd) {
    ...
    LEILEILEI_ASSERT(fd_ctx->events == 0);
    return true;
}
...
void IOManager::idle() {
    ...
    while(true) {
        ...
        //if(LEILEILEI_UNLIKELY(rt == MAX_EVNETS)) {
