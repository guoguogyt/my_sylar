...
void Scheduler::start() {
    ...
    m_stopping = false;
    LEILEILEI_ASSERT(m_threads.empty());
    ...
    //    //m_rootFiber->swapIn();
    //    m_rootFiber->call();
    //    leileilei_LOG_INFO(g_logger) << "call out " << m_rootFiber->getState();
    //}
}
    ...
    while(true) {
        ...
        bool is_active = false;
        {
            ...
            while(it != m_fibers.end()) {
                if(it->thread != -1 && it->thread != leileilei::GetThreadId()) {
                    ++it;
                    tickle_me = true;
                    continue;
                }

                LEILEILEI_ASSERT(it->fiber || it->cb);
...
void Scheduler::switchTo(int thread) {
    LEILEILEI_ASSERT(Scheduler::GetThis() != nullptr);
    if(Scheduler::GetThis() == this) {
        if(thread == -1 || thread == leileilei::GetThreadId()) {
            return;
        }
    }
