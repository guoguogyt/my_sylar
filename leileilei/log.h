 ...
 *  底层：  LogEvent日志事件          事件中有日志的所有信息，但是经过LogFormatter之后信息并不会全部输出，会按照指定的格式进行输出 
 ...
 遗留问题：
    多个线程写日志，不同的线程写的日志(logger)不一样，但是这些logger都输出到控制台或者是同一个文件，会造成输出混乱
    线程A   线程B   日志器L1    日志器L2
    L1和L2都输出到控制台
    锁加在了同一个日志器，既当A和B都想利用L1写日志时，会竞争锁，此时是有序的
    但是当A竞争L1，而B竞争L2时，A获取到了L1，B获取到了L2。
    此时A通过L1向控制台写日志，B通过L2向控制台写日志，此时会产生控制台写日志混乱
