 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2022-12-29 16:10:25
 */
...
{ \
    ...
    leileilei::ByteArray::ptr ba(new leileilei::ByteArray(base_len)); \
    for(auto& i:vec) \
    { \
        ba->write_fun(i); \
    } \
    ...
    { \
        type v = ba->read_fun(); \
        LEILEILEI_ASSERT(v == vec[i]); \
    } \
    LEILEILEI_ASSERT(ba->getReadSize() == 0); \
    LEI_LOG_DEBUG(g_logger) << #write_fun "/" #read_fun \
                    " (" #type " ) len=" << len \
                    << " base_len=" << base_len \
                    << " size=" << ba->getSize(); \
}
...
#define XX(type, len, write_fun, read_fun, base_len) {\
    ...
    LEILEILEI_ASSERT(ba2->getPosition() == 0); \
}
    XX(int8_t,  100, writeFint8, readFint8, 1);
    XX(uint8_t, 100, writeFuint8, readFuint8, 1);
    XX(int16_t,  100, writeFint16,  readFint16, 1);
