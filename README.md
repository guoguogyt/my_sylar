...
| 2022-12-20 | 封装Socket类。<br />观看完视频54p-56p，并实现对应的代码。<br />将socket类与Address类、FdManager类、IOManager类进行结合。可以实现socket通信功能。<br />创建Socket类并没有创建socket，只有在newSock的时候才会创建socket。<br />Socket类封装：util10 |
| 2022-12-29 | 封装ByteArray类。<br />观看完视频57p-60p，并实现对应的代码。<br />序列化类，可以将二进制数据转成对应的数据。也可以将不同类型的数据封装成二进制。<br />ByteArray类：util11 |
